...
def fillorderid():
    ...
    cur=db.cursor()
    sql="select orderid,custid,dateoforder,itemno,qty from orders"
    cur.execute(sql)
    data=cur.fetchall()
    for res in data:
        aa.append(res[0])
...
def firstdata():
    ...
    dateoforder_entry.insert(0,ac[i])
    dateoforder_entry.insert(0,ad[i])
...
dateoforder=Label(t,text="Date of Order -: ")
dateoforder.place(x=50,y=150)
itemno=Label(t,text='Item No -:')
itemno.place(x=50,y=200)
...
dateoforder_entry=Entry(t,width=25)
dateoforder_entry.place(x=250,y=150)
itemno_entry=Entry(t,width=25)
itemno_entry.place(x=250,y=200)
